...
<%  List<Student> student = (List)session.getAttribute("student"); 
 List<teacher> Listteacher = (List)session.getAttribute("Listteacher"); 
...
<%}else{ %>
	...
	<div class="container" style="margin-top: 35px;">
		...
		<section id="content">
			<div class="container" style="margin-top: -20px">
				<div class="row">
					<div class="col-lg-12">
					 <form name="frm" method="post" action="${pageContext.request.contextPath}/SearhAllAnswerTeacher" >
						<div class="container">
	...
	<div class="container" style="margin-top: 35px;">
    <div align="right">
        <a href="${pageContext.request.contextPath}/ExportAllAnswerTeacher?searchDate=<%=getSemester%>" class="btn btn-primary">
            <i class="fa-solid fa-file-export"></i> Export Excel File
        </a>
    </div>
    <h3><span style="color:#006ebc;">ผลการประเมินสหกิจศึกษาของอาจารย์นิเทศ</span> <%=getSemester%> </h3>
    <h5>จำนวนนักศึกษา <%=student.size() %> คน </h5>
    <hr>

    <table class="table table-bordered" id="myTable">
        <thead class="table-info">
            <tr align="center">
                <th>อาจารย์นิเทศสหกิจศึกษา</th>
                <th>บริษัท</th>
                ...
                <% if (Listteacher != null && !Listteacher.isEmpty()) { %>
                            ...
                            <% for (Student S : student) { %>
                                ...
                                <% if (tans != null && !tans.isEmpty()) { %>
                                    <% for (teacheranswer ta : tans) { %>
                                        <% answeredQuestions.add(ta.getQuestionid_answer()); %>
                                    <% } %>
                                <% } %>
                            <% } %>
                        <% } %>
                    ...
                    <% for (question que : ListQ) { %>
                        <% if (answeredQuestions.contains(Integer.parseInt(que.getQuestionid()))) { %>
                            <th><%=contquestion%>. <%= que.getQuestiontext() %></th>
                            <%contquestion = contquestion+1; %>
                        <% } %>
                    <% } %>
                <% } else { %>
                    <th colspan="6" align="center"><h5 style="color:#850000"><i class="fa fa-ban"></i> ไม่มีข้อมูลนักศึกษา </h5></th>
                <% } %>
            </tr>
        </thead>
        <tbody>
                    ...
                    <% if (student != null && !student.isEmpty()) { %>
                        <% for (Student S : student) { %>
                            ...
                            <% if (tans != null && !tans.isEmpty()) { %>
                                ...
                                <tr align="center">
                                    <td><%= t.getTeachername() %> <%= t.getTeacherlastname() %></td>
                                    <td><%= com.getCompanyname() %></td>
                                    <td><%= S.getIdstudent() %> <%= S.getStudentname() %> <%= S.getStudentlastname() %></td>
                                    <% for (question que : ListQ) { %>
                                        <% if (answeredQuestions.contains(Integer.parseInt(que.getQuestionid()))) { %>
                                            <% boolean answered = false; %>
                                            <% for (teacheranswer ta : tans) { %>
                                                <% if (Integer.parseInt(que.getQuestionid()) == ta.getQuestionid_answer()) { %>
                                                    <td><%= ta.getScore() %></td>
                                                    <% answered = true; %>
                                                <% } %>
                                            <% } %>
                                            <% if (!answered) { %>
                                                <td>-</td> <!-- หรือช่องว่างแทน "-" ถ้าต้องการ -->
                                            <% } %>
                                        <% } %>
                                    <% } %>
                                </tr>
