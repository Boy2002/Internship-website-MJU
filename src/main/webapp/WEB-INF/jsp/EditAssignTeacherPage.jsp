...
<%
    ...
    <div class="alert">
        ...
        วันที่ <%=errordate%> ได้กำหนดการนิเทศสหกิจศึกษาทั้ง2ช่วงเวลาแล้ว (ช่วง 10:00 ช่วง 13:00)
    </div>
	...
	<div align="center">
		...
		%>
		<form name="frm" method="post"
			action="${pageContext.request.contextPath}/EditassignSupervision">
			<table class="table table-bordered" id="myTable" style="width: 80%">
				...
				<% AssisgnTeacherDB asDB = new AssisgnTeacherDB();
					Assignsupervision as = asDB.assignsupervisionBycomidAndsemester(Com.getCompanyid(),getSemester);		
				...
				%>
					<tr>
						...
						<th>
                        <input type="date" name="datesupervision" class="form-control data" id="dateAssign" min="<%= minDateString %>" value="<%= as.getDate() %>" required="required">
                    	</th>
						<th style="white-space: nowrap">
					    <select name="timeSupervision" class="form-control data" id="timeAssign">
					        <% 
					        String selectedTime = as.getTime(); // รับค่าเวลา
					        %>
					        
					        <option value="10:00น.-12.00น." <%= "10:00น.-12.00น.".equals(selectedTime) ? "selected='selected'" : "" %>>10:00น.-12.00น.</option>
					        <option value="10:30น.-12.00น." <%= "10:30น.-12.00น.".equals(selectedTime) ? "selected='selected'" : "" %>>10:30น.-12.00น.</option>
					        <option value="11:00น.-12.00น." <%= "11:00น.-12.00น.".equals(selectedTime) ? "selected='selected'" : "" %>>11:00น.-12.00น.</option>
					        <option value="13:00น.-15.00น." <%= "13:00น.-15.00น.".equals(selectedTime) ? "selected='selected'" : "" %>>13:00น.-15.00น.</option>
					        <option value="13:30น.-15.00น." <%= "13:30น.-15.00น.".equals(selectedTime) ? "selected='selected'" : "" %>>13:30น.-15.00น.</option>
					        <option value="14:00น.-15.00น." <%= "14:00น.-15.00น.".equals(selectedTime) ? "selected='selected'" : "" %>>14:00น.-15.00น.</option>
					    </select>
					</th>
						...
						<th style="white-space: nowrap"><select name="teacherSupervision" class="form-control data">
								<%if (Lteacher != null) {%>						
								<%for (teacher t : Lteacher) {%>
								<%if(Integer.parseInt(as.getTeacherid()) == t.getTeacherid()) {%>
								<option value="<%=t.getTeacherid()%>" selected="selected"><%=t.getTeachername() + " " + t.getTeacherlastname()%></option><%}else{ %>								
						...
						<th><select name="methodSupervision" class="form-control data">
						<%if("On-Site".equals(as.getMethods())){ %>
								<option value="On-Site" selected="selected">On-Site</option>
								<option value="On-Line">On-Line</option>
								<%}else{ %>
								<option value="On-Site">On-Site</option>
								<option value="On-Line" selected="selected">On-Line</option>
								<%} %>
						</select>
						</th>
						<%} %>
