...
Student stu = null;
List<formmenterdetail> Listfd = null;
List<mentoranswer> Listans  = null;
try{
	mf = (Mentorassessmentform) session.getAttribute("mf");
...
<body>
        ...
        <div class="container">
            <div class="row no-gutter">
                <div class="col-xs-12 col-md-12"
                    ...
                    <h3 style="color: #7EBC1B;">ระบบประเมินผลสหกิจศึกษาสำหรับพนักงานพี่เลี้ยง</h3>
    ...
    <script>
        ...
        <%}else if (formnotfound == 1 || mf == null){ %>
     <script>
        Swal.fire({
            icon: 'warning',
	...
	Listfd = cfDB.getformmenterdetailById(mf.getMentorassessmentformid());
	//////คำตอบของพี่เลี้ยง
	AssessmentDB asmDB = new AssessmentDB();
	System.out.println(mentor.getMentorid()+" "+stu.getIdstudent());
	Listans = asmDB.CheckAssessmentMentorAnswer(mentor.getMentorid(),stu.getIdstudent());
	%>
    ...
    <div class="container" style="width: 65%">
        <br>
       <!--  <h3 style="color: #850000;">ระบบประเมินผลสหกิจศึกษาสำหรับพนักงานพี่เลี้ยง</h3>
        <hr class="style13"> -->
		
		<div class="background-A4">
		<div style="margin-left: 10%;" >
                    <h4 style="width: 100%;"><%= mf.getTitle() %></h4>
                    
                     <div class="container-headerform">
					    <div class="left label-group-headerform">
					        <span style="font-size: 18px; "><b>ผู้ประเมิน :</b></span>
                    		<span style="font-size: 18px;"><%= mentor.getMentorname() %> <%= mentor.getMentorlastname() %> </span><br>
                    		<br>
      ...
      <div style="background-color: #F9F9F9; border-radius: 15px; padding: 4%; box-shadow: 10px 10px 20px rgba(0, 0, 0, 0.2); ">
            <form action="EditAssessmentMentor" method="post" onsubmit="return AlertEdit(event)">
                <input type="hidden" id="rowCount" name="rowCount" value="0">
                <input type="hidden" id="totalscorevalue" name="totalscorevalue">
                <input type="hidden" id="mentorid" name="mentorid" value="<%=mentor.getMentorid()%>">
                 <input type="hidden" id="stuid" name="stuid" value="<%=stu.getIdstudent()%>">
                <input type="hidden" id="mentorformid" name="mentorformid" value="<%=mf.getMentorassessmentformid()%>">
                ...
                <!-- <h6><b>&nbsp;&nbsp;&nbsp;&nbsp; แบบฟอร์มการประเมินสหกิจศึกษาสำหรับพนักงานพี่เลี้ยง คะแนนเต็มคิดเป็นร้อยละ 60 คะแนน</b></h6> -->
               ...
               <div class="table-header">
                ...
                <div id="questionContainer">
                 ...
                 <div align="right" style="margin-right: 50px;">
                    <a onclick="updateTotalScore()" style="cursor: pointer;">
                        <span id="totalscore" style="font-size: 18px;"></span><span style="font-size: 18px;">/<%= Math.round(mf.getTotalscore())%></span>
                        <input type="hidden" id="mentortotalscore" name="mentortotalscore" value="0">
				...
				<hr>
				สาขาวิชาเทคโนโลยีสารสนเทศ ขอขอบพระคุณบริษัท ผู้ควบคุมดูแล/พี่ลี้ยง รวมถึงผู้ที่เกี่ยวข้อง ทุกท่าน ที่ได้ให้ความอนุเคราะห์รับและดูแลนักศึกษา เข้าปฏิบัติงานสหกิจศึกษา อีกทั้งยังได้ให้ประเมินผลการปฏิบัติงานสหกิจศึกษาที่เป็นประโยชน์ในครั้งนี้
				<hr>
               <br>
               <br>
                <br>
                <div align="center">
                    <button type="submit" class="btn btn-success" style="margin: 20px; width: 150px;">แก้ไข</button>
                    <a href="${pageContext.request.contextPath}/loadlistStuMentorPage?mid=<%=mentor.getMentorid() %>" class="btn btn-warning" style="margin: 20px; width: 150px;">ยกเลิก</a>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    <% if (Listans != null) {
        for (mentoranswer ans : Listans) { %>
            answerData["<%= ans.getQuestionid_ans() %>"] = {
                anstext: "<%= ans.getMentoranswertext() %>"
            };
    <% } 
    } %>
        ...
        <% for (formmenterdetail fd : Listfd) { %>
            ...
            if (questionDataObj) {
               ...
               /*  var questionLabel = document.createElement("label");
                ...
                // สร้างพื้นที่ใส่ input ขึ้นอยู่กับประเภทคำถาม
             ...
             if (answerDataObj) {
                 ...
                 if (questionType === "ใช่หรือไม่") {
                     ...
                     questionElement = `
                    	 ...
                    	 <label><input type="radio" name="questions[` + rowCount + `].answer" value="ใช่" data-score="` + questionScore + `" ` + (answerText === "ใช่" ? 'checked' : '') + ` required> ใช่</label>
                         <label><input type="radio" name="questions[` + rowCount + `].answer" value="ไม่ใช่" data-score="0" ` + (answerText === "ไม่ใช่" ? 'checked' : '') + `> ไม่ใช่</label>
                         </div>`;
                 } else if (questionType === "กรอกคะแนน") {
                     // แสดง input สำหรับกรอกคะแนน ด้านข้างคำถาม
                     questionElement = `
                    	 <input type="number" min="1" max="` + questionScore + `" class="form-control question-input inline-input" name="questions[` + rowCount + `].answer" data-score="` + questionScore + `" value="` + answerText + `" required>`;
                 } else if (questionType === "ข้อความ") {
                     // แสดง textarea ด้านล่างคำถาม
                     questionElement = `
                    	 <textarea class="form-control full-width below-input" name="questions[` + rowCount + `].answer" rows="3" data-score="` + questionScore + `" >` + answerText + `</textarea>`;
                 }
...
function updateTotalScore() {
    ...
    var mentortotalScoreElement = document.getElementById("mentortotalscore");
    mentortotalScoreElement.value = totalScore;
}
</script>

<script>
function AlertEdit(event) {
    event.preventDefault(); // หยุดการ submit ชั่วคราว
    Swal.fire({
        icon: 'warning',
        title: 'สามารถแก้ไขได้อีก 1 ครั้งต้องการแก้ไขหรือไม่',
        /* text: "คุณสามารถแก้ไขได้อีก 1 ครั้งต้องการแก้ไขหรือไหม", */
        showDenyButton: true,
        confirmButtonText: 'ยืนยัน',
        denyButtonText: 'ยกเลิก'
    }).then((result) => {
        if (result.isConfirmed) {
            event.target.submit(); // ส่งฟอร์มเมื่อยืนยัน
        }
    });
}
