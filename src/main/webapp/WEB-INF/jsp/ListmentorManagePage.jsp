...
<%
try{
	sendemail = (int)session.getAttribute("sendemail");
}catch(Exception e) {
	sendemail = 0;
...
Swal.fire({
  position: "center",
  icon: "success",
  title: "ส่งบัญชีเข้าใช้งานเรียบร้อยแล้ว",
...
<div class="alert">
  <span class="closebtn">&times;</span>  
  <strong> <i class="fa-sharp fa-solid fa-circle-xmark"></i>ข้อมูลไม่สำเร็จ : </strong> กรุณาตรวจสอบข้อมูลพี่เลี้ยง 
</div>
...
<%}else{ %>
<br><br><% List<String> semester =  ListStu.AllListsemester(); %>
					 <form name="frm" method="post" action="${pageContext.request.contextPath}/loadSearchListmentorManage" >
						<div class="container">
							<h3 style="color:#850000;" >ข้อมูลพนักงานพี่เลี้ยง</h3>
							<hr class="style13">
							<input type="text" class="main-input main-name" name="NAME" placeholder ="ชื่อพี่เลี้ยง" onfocus="clearText(this)" onblur="replaceText(this)" />                                  
	...
	<div class="container" style="margin-top: 35px;">
				...
				<a href = "${pageContext.request.contextPath}/loadListmentorNostudentManage" class="btn btn-primary" ><i class="fa-solid fa-person"></i> พี่เลี้ยงที่ไม่มีนักศึกษาในการดูแล </a>
				</div>
				<br>
                <table class="table table-bordered" id="myTable">
                    ...
                    <%for(Mentor MM : listmentor){ %>
                     <%
							        ...
							        String encryptedPassword = sm.getEncryptedPasswordByUsername(MM.getMetoremail()); 
							        String encodedKey = sm.getEncryptedKeyByUsername(MM.getMetoremail()); 
							        ...
							        decryptedPassword = AESpassword.decrypt(encryptedPassword, secretKey);
							       	...
							       	decryptedPassword = MM.getPassword();
							        }
						...
						<th align="center" style="text-align: center;">
						    <a href="${pageContext.request.contextPath}/sendEmail?to=<%=MM.getMetoremail()%>&subject=Email Mentor for Assessment&content=เรียน%20คุณ <%=MM.getMentorname() %> <%=MM.getMentorlastname() %>,%0A%0Aเราขอแจ้งให้ทราบว่าเว็บไซต์สหกิจศึกษาของสาขาเทคโนโลยีสารสนเทศ%20มหาวิทยาลัยแม่โจ้เชียงใหม่%20ได้ดำเนินการจัดส่งนักศึกษาไปฝึกสหกิจศึกษาตามบริษัทที่กำหนดเรียบร้อยแล้ว%0A%0Aหลังจากการฝึกสหกิจศึกษาเสร็จสิ้น%20ทางเราขอให้พี่เลี้ยงทำการล็อกอินเข้าสู่เว็บไซต์เพื่อทำการประเมินนักศึกษา%20โดยรายละเอียดการเข้าล็อกอินจะถูกส่งไปยังอีเมล์ที่นักศึกษาได้รับ%0A%0Aรายละเอียดการเข้าล็อกอิน:%0Aอีเมล์:%20<%=MM.getMetoremail()%>%0Aรหัสผ่าน:%20<%=decryptedPassword%>%0Aเว็บไซต์สหกิจศึกษาสาขาเทคโนโลยีสารสนเทศ:%20https://itsci.mju.ac.th/coopedu/%0A%0Aหากมีข้อสงสัยหรือต้องการความช่วยเหลือเพิ่มเติม%20สามารถติดต่อได้ที่ฝ่ายสนับสนุนของเว็บไซต์.%0A%0Aขอบคุณมากค่ะ/ครับ,%0Aทีมงานสหกิจศึกษามหาวิทยาลัยแม่โจ้เชียงใหม่">
