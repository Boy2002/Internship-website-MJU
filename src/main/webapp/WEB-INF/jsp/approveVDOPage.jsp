...
<%
 ...
 List<VDO> VDOList = (List)session.getAttribute("VDOList");
...
<div class="nav1" style="color:#FFFFFF;"><a class="a2" href="#" style="color:#E28A06;">อนุมัติวิดีโอ </a></div>
...
<div class="container">						
	...
	<h5 style="color:#850000;"> ข้อมูลวิดีโอรายงานสหกิจ </h5>
	<hr class="style13">
</div>
...
<%for(VDO RV : VDOList){ %>
<% String[] arr = RV.getFilename().split("https://youtu.be/"); %>
<%Company cp = ListCompany.Searchcompanyid(RV.getCompany_companyid() );%>
<div class="container position-relative">
  <div class="card mt-4 mb-4">
    <div class="card-body">
          ...
          <div class="row">
            <div class="hover-name col-auto pr-0">
              ...
              <%//Student S = STUDB.SStuid(RV.getStudent_studentid()); %>
                ...
                <h5>บริษัท <%=cp.getCompanyname()%></h5>         
              </div>
            </div>
            <div class="col-auto pr-0">        
              <span class="text-muted"><i class="fa-sharp fa-solid fa-earth-americas"></i> <%=RV.getSentdate()%></span>   
              <a href="${pageContext.request.contextPath}/EdApproveVDOPage?getstatus=1&getvideoid=<%=RV.getVideoid() %>" style = "margin-left:200px"><button class="btn btn-danger" ><i class="fa-solid fa-circle-xmark" style="font-size:17px"> ไม่อนุมัติให้เผยแพร่ </i></button></a>  
              <a href="${pageContext.request.contextPath}/EdApproveVDOPage?getstatus=2&getvideoid=<%=RV.getVideoid() %>" ><button class="btn btn-success" ><i class="fa-solid fa-circle-check" style="font-size:17px"> อนุมัติให้เผยแพร่ </i></button></a>         
            </div>
          </div>
      ...
      <hr>
      <div class="media-footer" align = "center">
       <%if(RV.getFilename().contains("https://youtu.be/")){ %>
     			<iframe width="450" height="400" src="https://www.youtube.com/embed/<%=arr[1]%>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>					
       <%}else{ %>
        <h5>*URL youtube ไม่ถูกต้องกรุณาส่งใหม่ </h5>
       <%} %> 
      </div>
    </div>
  </div>
