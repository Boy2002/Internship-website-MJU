...
try{
	CheckQuestion = (int)request.getAttribute("CheckQuestion");
}catch(Exception e) {
	CheckQuestion = 0;
	}
try{
	questiontid = (int)request.getAttribute("questiontid");
}catch(Exception e) {
	questiontid = 0;
	}
%>
...
<%
try{
	formNames = (List)request.getAttribute("formNames");
}catch(Exception e) {
	formNames = null;
	}
...
<h3 style="color:#7EBC1B;" >ระบบจัดการคำถามแบบฟอร์มสหกิจศึกษา </h3>
<div class="nav1" style="color:#FFFFFF;"><a class="a2" href="#" style="color:#E28A06;">จัดการคำถาม </a></div>
...
<div class="alert success">
  ...
  <strong> <i class="fa-sharp fa-solid fa-circle-check"></i> เพิ่มข้อมูลสำเร็จ : </strong> เพิ่มข้อมูลเรียบร้อยแล้ว  
</div>
<%} %>
<%if(error == -1){ %>
     <script>
            Swal.fire({
                icon: 'error',
                title: 'มีการใช้งานคำถามในฟอร์มอื่น! ไม่สามารถลบได้',
                /* html: '<p>รายการชื่อฟอร์มที่ใช้คำถาม:</p><ul style="display: inline-block; text-align: left;">' + 
...
<% if (CheckQuestion == 1) { %>
    <script>
        Swal.fire({
            icon: 'warning',
            title: 'มีการใช้คำถามในฟอร์มอื่น ไม่สามารถแก้ไขได้',
            /* showDenyButton: true, */
           /*  html: '<p>รายการชื่อฟอร์มที่ใช้คำถาม:</p><ul style="display: inline-block; text-align: left;">' + 
                  '<c:forEach items="${formNames}" var="formName">' +
                  '<li>${formName}</li>' +
			...
			<%-- confirmButtonText: '<a href="${pageContext.request.contextPath}/editquestion?qid=<%=questiontid%>" style="color:#FFFFFF">ตกลง</a>', --%>
        	confirmButtonText: 'ปิด',
        });
    </script>
<% } %>
		...
		<section id="content">
			<div class="container" style="margin-top: -20px">
				<div class="row">
					<div class="col-lg-12">
						<div class="container">
							<h3 style="color:#850000">คำถามทั้งหมด จำนวน <%=Listq.size() %> ข้อ</h3>
							<hr class="style13">
					 ...
					 <div style="background-color: white; border-radius: 15px; padding: 50px;  box-shadow: 10px 10px 20px rgba(0, 0, 0, 0.2);">
                           ...
                           <div align="right" style="width: 100%; margin-right: 20px; margin-bottom: 20px;">
                            <a href="${pageContext.request.contextPath}/addquestion"
						class="btn btn-primary"><i class="fa-solid fa-plus"></i> เพิ่มคำถาม </a>
						</div>
					...
					<table class="table table-bordered" id="myTable" style="width:100%;">
								...
								<%for(question qt : Listq) {%>
									...
									<tr>
				                		<td align="center"><%=qt.getQuestionid()%></td>
				                		<td style="padding-left: 30px;"><%=qt.getQuestiontext()%></td>
				                
				                		<td align="center"><a href="${pageContext.request.contextPath}/ListFormByQuestionId?qid=<%=qt.getQuestionid()%>"
								        ...
								        List<formteacherdetail> ftd = aqDb.CheckQuestionInFormTeacher(Integer.parseInt(qt.getQuestionid()));
								        List<formmenterdetail> fmd = aqDb.CheckQuestionInFormMentor(Integer.parseInt(qt.getQuestionid()));
										%>
										<%if (ftd.isEmpty() && fmd.isEmpty() ){ %>
										<button class="btn btn-danger" onclick="alertDelete(<%=qt.getQuestionid()%>)">ลบ</button>
										<%}else{ %>
										 <a href="${pageContext.request.contextPath}/DeleteQuestion?qid=<%=qt.getQuestionid()%>"
				                ...
				                <%}else{ %>
				                 <tr>
                    	            <td colspan = "6" align="center"><h5 style="color:#850000"> <i class="fa fa-ban"></i> ไม่มีข้อมูลคำถาม </h5></td>
                                  </tr>
                                  <%} %>  
								</tbody>
							</table>
...
<script>
function alertDelete(questionid) {
    Swal.fire({
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            window.location.href = '${pageContext.request.contextPath}/DeleteQuestion?qid=' + questionid;
        }
    });
}
</script>
